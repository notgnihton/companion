...
describe("RuntimeStore - Pending Chat Actions", () => {
  ...
  it("prunes expired pending actions", () => {
    store.createPendingChatAction(userId, {
    ...
    expect(store.getPendingChatActions(userId, now)).toHaveLength(0);
  });
});
